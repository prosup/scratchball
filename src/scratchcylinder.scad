...
//shell 
    ...
    difference() 
        ...
        translate([0,shell_outer_radius/2,roller_l/2+joint_hight]) 
            cube([(shell_outer_radius-shell_thick)*2,shell_outer_radius+4,roller_l+2*joint_hight-2*shell_thick+0.6],true);
        }
    };
...
module slide()
{
    hull() 
    polyhedron([
    [joint_radius,0,joint_hight-0.4],//0
...
module socked_shell()
{
    ...
    difference() 
    {
        half_round_shell();
        translate([0,0,0.4]) 
        joint(joint_radius+0.2, joint_hight*2+roller_l-0.8);
    };
    union()
    {
        //bottom slide
